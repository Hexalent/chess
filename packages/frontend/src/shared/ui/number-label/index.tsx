...
export const NumberLabel = ({ cell }: NumberLabelProps) => {
  ...
  const numberLabelPosition = boardStyle === BoardStyle.INSIDE_BOARD ? 'top-0.5 left-1' : 'top-5 -left-4'

  if (!isNumberLabelVisible) return null

  return <div className={`number-label absolute text-sm font-bold text-gray-600 ${numberLabelPosition}`}>{cell.y}</div>
}
