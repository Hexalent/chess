...
export const LetterLabel = ({ cell }: LetterLabelProps) => {
  ...
  const letterLabelPosition =
    boardStyle === BoardStyle.INSIDE_BOARD ? 'top-[44px] left-[52px]' : 'top-[68px] left-[26px]'

  if (!isLetterLabelVisible) return null

  return (
    <div className={`letter-label absolute text-sm font-bold text-gray-600 ${letterLabelPosition}`}>
